...
Dans le cas d’une espèce $X$ en solution aqueuse, l’activité est définie par la relation :
...
Dans le cas d’une espèce $X$ qui est un solvant liquide ou un solide, l'activité est définie par : $a_{X(l,solvant)}=1$ et $a_{X(solide)}=1$
...
1. $2H_2O_{\textcolor{red}{(l)}} ⇌ HO_{(aq)}^- + H_3O_{(aq)}^+$
...
Remarques : la constante d’équilibre est sans dimension et ne dépend que de la température, d’où sa notation $K(T)$. L’indice _eq_ signifie équilibre.
